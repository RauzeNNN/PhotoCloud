...
public class profileListPage extends JFrame implements ActionListener{
	
	/*
	 * A page which contains all users public info and linked to all profile pages.
	 */
	 ...
	 * Create the frame.
	 */
	public profileListPage(User user, DataPack datapack) {  //rearrange this as posts
		this.user = user;
		this.datapack = datapack;
		Logger.traceLogger("User:" + user.getNickname() + " has opened profiles page.");
	    ...
	    int count = datapack.getUsers().size();
	    GridBagLayout gbl_middlePanel = new GridBagLayout();
	    gbl_middlePanel.columnWidths = giveArr(381, count);
	    gbl_middlePanel.rowHeights = giveArr(90, count); 
	    ...
	    int i = 0;
	    for (User profile : datapack.getUsers()) {
	    	if (profile == user) {
	    		continue;
	    	}
	    	JPanel panel = new ProfilePanel(user, profile, datapack);
